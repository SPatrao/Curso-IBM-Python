...
def generar_certificado_final(usuario, progreso_usuarios):
    ...
    try:
        ...
        ruta_certificado = os.path.join(ruta_certificados, f"certificado_final_{usuario}.pdf")
        ...
        # Agregar las categorías completadas

        for categoria, completado in progreso_usuarios[usuario].items():

            if completado:

                elements.append(Paragraph(f"- Categoría: {categoria}", styles["BodyText"]))
...
def actualizar_progreso(usuario, categoria, modulo, progreso_usuarios):
    ...
    if progreso_usuarios[usuario][categoria]:
    ...
    if all(progreso_usuarios[usuario].values()):
        ...
        generar_certificado_final(usuario, progreso_usuarios)
